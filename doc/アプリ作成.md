...
## Todoアプリケーションの作成
### 1.todoアプリケーションを作成するため、以下のコマンドを実行してください。
```
$ python3 manage.py startapp todo
```

### 2.Todoアプリケーションの組み込み設定
mysite/settings.pyを編集し、INSTALLED_APPSと書かれている部分に「'django.contrib.admin',」を以下のように追加してください。
```
INSTALLED_APPS = [
...
### 4.Todoアプリケーションのフォーム作成
Todoアプリケーションのフォームを作成します。入力項目をToDoのみにするため、forms.pyに以下のコードの追加をお願いします。
```
...
### 5.Todoアプリケーションのビュー作成
todo/views.pyに以下のコードを追加してください。
```
...
### 6.TodoアプリケーションのURLを設定
TodoアプリケーションのURL設定を行います。todo/でアクセスされた際にtodoアプリケーションのURL設定を参照するように以下のように記載します。
↓ mysite/urls.py
```
...
Todo/templates/base.htmlにhtmlのベースとなるテンプレートを作成します。
```
{% load static %}
<!DOCTYPE html>
<html lang="ja">
...
Todoの一覧表示機能のための以下のコードをtodo/templates/todo/todo_list.htmlに追加お願いします。
```
{% extends 'base.html' %}
...
### 8.Todoの詳細を表示させる
Todoの詳細を表示させるコードとして以下のコードをtodo/templates/todo/todo_detail.htmlに追加してください。
```
{% extends 'base.html' %}
...
Todoの作成機能とTodoの編集機能を実装するため、todo/templates/todo/todo_form.htmlに以下のコードを追加お願いします。
```
{% extends 'base.html' %}
...
```
### 10.Todoの削除機能実装
Todoの削除のためにtodo/templates/todo/todo_confirm_delete.htmlに以下のコードの追加をお願いします。
```
{% extends 'base.html' %}
...
### 参考サイト
[これです](https://qiita.com/acecrc/items/a4302ca6cf0897c45302)<br>
